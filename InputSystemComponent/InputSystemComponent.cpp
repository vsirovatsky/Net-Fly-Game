// InputSystemComponent.cpp : Defines the entry point for the console application.
//
...
int main(){
	std::cout << "\n\n*** My OIS Console Demo App is starting up... *** \n";
	try {
		...
		inputsystem::InputSystemFacade * isf = inputsystem::InputSystemFacade::INSTANCE();
		isf->init(hWnd, 1000,1000,true,true,false);
		inputsystem::MouseState * ms = new inputsystem::MouseState();
		inputsystem::KeyboardBufferedState * kbs = new inputsystem::KeyboardBufferedState();
		inputsystem::KeyboardUnbufferedState * kus = new inputsystem::KeyboardUnbufferedState();
		while(appRunning){
			Sleep( 50 );
			isf->capture( ms, kbs, kus );
			if ( kbs->keysReleased[inputsystem::KC_ESCAPE] == 1 ) {
				appRunning = false;
			}
			for ( int i = 0; i < 256; i++ ) {
				// Keyboard
				if ( kbs->keysPressed[i] == 1 ) {
					kbs->keysPressed[i] = 0;
					std::cout << "  (" << i << ") ";
				}
				if ( kus->keys[i] == 1 ) {
				...
				if ( kbs->keysReleased[i] == 1 ) {
					kbs->keysReleased[i] = 0;
					std::cout << "  [" << i << "] ";
				}
			}
			// Mouse
			for ( int i = 0; i < 8; i++ ) {
				if (ms->click.clicksDown[i].click) {
					std::cout << "  (" << (i==0?"LEFT":(i==1?"RIGHT":(i==2?"MIDDLE":"OTHER"))) << ") ";
					ms->click.clicksDown[i].clear();
				}
			}
			if (ms->isButtonDown( inputsystem::MB_Left )) {
				std::cout << "  '" << "LEFT" << "' ";
			}
			if (ms->isButtonDown( inputsystem::MB_Right )) {
				std::cout << "  '" << "RIGHT" << "' ";
			}
			if (ms->isButtonDown( inputsystem::MB_Middle )) {
				std::cout << "  '" << "MIDDLE" << "' ";
			}
			for ( int i = 0; i < 8; i++ ) {
				if (ms->click.clicksUp[i].click) {
					std::cout << "  [" << (i==0?"LEFT":(i==1?"RIGHT":(i==2?"MIDDLE":"OTHER"))) << "] ";
					ms->click.clicksUp[i].clear();
				}
			}
			//std::cout << "(" << ms->moveUnbuffered.xAbs <<","<< ms->moveUnbuffered.yAbs <<","<< ms->moveUnbuffered.zAbs << ")" << "\n";
			std::cout << "(" << ms->moveBuffered.xAbs <<","<< ms->moveBuffered.yAbs <<","<< ms->moveBuffered.zAbs << ") [" << ms->moveBuffered.xRel <<","<< ms->moveBuffered.yRel <<","<< ms->moveBuffered.zRel << "]\n";
			ms->moveBuffered.clear();
		}
